...
export class BaseService<M> {
  ...
  protected sendGet<M>(url: any, options?: any): Observable<M> {
  ...
  protected sendPut<M>(url: any, payload?: any, options?: any): Observable<M> {
    return this.httpClient.put<M>(url, payload, options).pipe(
      map((body: any) => body),
      catchError(this.handleError)
    );
  }
  protected sendDelete<M>(url: any, options?: any): Observable<M> {
